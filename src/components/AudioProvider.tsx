...
function getThemeForPath(pathname: string): string {
    ...
    if (pathname.startsWith('/multiplayer/')) return 'stadium'; // Sub-rooms use stadium
    if (pathname.startsWith('/multiplayer')) return 'stadium';  // Main lobby
    if (pathname.startsWith('/algo-quest')) return 'stadium';   // Algo Dojo is intense!
    return 'menu'; // home, block-adventure map, trophies, etc.
}
